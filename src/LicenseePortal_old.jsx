...
const LicenseePortal = () => {
  ...
  const loadFieldMetadata = async () => {
    try {
      // Load the Thentia JSON metadata
      const response = await fetch('/azbtr_tc_entitymetadata.json');
      const data = await response.json();
      
      // Extract application fields from the metadata
      const applicationFields = extractApplicationFields(data);
      setFieldMetadata(applicationFields);
      setLoading(false);
    } catch (error) {
      console.error('Error loading field metadata:', error);
      // Fallback to basic fields if JSON loading fails
      setFieldMetadata(getBasicApplicationFields());
      setLoading(false);
    }
  };
